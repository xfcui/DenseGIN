...
SCALE_GRAPH, SCALE_NODE, SCALE_EDGE = 4, 4, 4
...
def _segment_sum(x, segment_ids, num_segments, mask):
    # x: (N, D), segment_ids: (N,), num_segments: int, mask: (N,)
    x = x * mask[:, None].astype(x.dtype)
    return jax.ops.segment_sum(x, segment_ids, num_segments)

def _segment_max(x, segment_ids, num_segments, mask):
    # x: (N, D), segment_ids: (N,), num_segments: int, mask: (N,)
    x = jnp.where(mask[:, None], x, -np.inf)
    return jax.ops.segment_max(x, segment_ids, num_segments)

def _segment_logsumexp(x, segment_ids, num_segments, mask):
    # x: (N, D), segment_ids: (N,), num_segments: int, mask: (N,)
    max_x = _segment_max(x, segment_ids, num_segments, mask)
    stable_x = x - max_x[segment_ids]
    sum_exp = _segment_sum(jnp.exp(stable_x), segment_ids, num_segments, mask).clip(1, None)
    return jnp.log(sum_exp) + max_x
...
class DegreeLayer(eqx.Module):
    ...
    def __init__(self, num_head, power_init=-.5):
    ...
    def __call__(self, x, deg):
        ...
        # deg: (N,)
        power = jnp.repeat(self.power.clip(None, 0), DIM_HEAD)
...
class GroupLinearUnit(eqx.Module):
    """Groups-parallel linear: weight (num_head, d_in, d_out), no bias."""
    num_head: int
    linear: LinearLayer
    kernel: jnp.ndarray

    def __init__(self, width_in, width_out, num_head, key):
...
class GatedLinearUnit(eqx.Module):
    ...
    num_head: int
    gate:  GroupLinearUnit
    value: GroupLinearUnit
    kernel: jnp.ndarray
    linear: LinearLayer | None

    def __init__(self, width_in, width_out, num_head, scale_act, keep_groups=False, key=None):
        width_norm = num_head * DIM_HEAD
        ...
        self.num_head = num_head
        self.gate  = GroupLinearUnit(width_in, width_act, num_head, keys[0])
        self.value = GroupLinearUnit(width_in, width_act, num_head, keys[1])
        if keep_groups:
            assert width_out % num_head == 0
...
class MetaFormerUnit(eqx.Module):
    sca_pre:  ScaleLayer
    sca_post: ScaleLayer
    glu_pre:  GatedLinearUnit
    glu_post: GatedLinearUnit

    def __init__(self, width, num_head, scale_act, key=None):
        keys = _split_or_none(key, 2)

        self.sca_pre  = ScaleLayer(width, scale_init=1.0)
        self.sca_post = ScaleLayer(width, scale_init=1.0)
        self.glu_pre  = GatedLinearUnit(width, width, num_head, scale_act, key=keys[0])
        self.glu_post = GatedLinearUnit(width, width, num_head, scale_act, key=keys[1])
    ...
    def __call__(self, x, msg, key=None):
        ...
        xx = self.sca_post(xx) + self.glu_post(xx, key=keys[1])
        return xx
...
class ConvBlock(eqx.Module):
    ...
    def __init__(self, width, num_head, edge_total_vocab, edge_num_features, key=None):
        width_norm = num_head * DIM_HEAD
        keys = _split_or_none(key, 2)

        self.embed = EmbedLayer(edge_total_vocab, edge_num_features, width_norm, keys[0])
        self.glu = GatedLinearUnit(width, width_norm, num_head, SCALE_EDGE, keep_groups=True, key=keys[1])
        self.deg = DegreeLayer(num_head)
    ...
    def __call__(self, x, deg, edge_idx, edge_attr, edge_mask, key=None):
        ...
        msg = _segment_sum(msg, edge_idx[1], len(x), edge_mask)
        msg = self.deg(msg, deg)
        return msg

# GIN-virtual: https://arxiv.org/abs/2103.09430
class VirtBlock(eqx.Module):
    """Virtual node aggregation for global information exchange."""
    num_head: int
    linear: GroupLinearUnit
    kernel: jnp.ndarray
    glu:    GatedLinearUnit

    def __init__(self, width, num_head, key=None):
        width_norm = num_head * DIM_HEAD
        width_act  = num_head * DIM_HEAD * SCALE_GRAPH
        keys = _split_or_none(key, 3)

        self.num_head = num_head
        self.linear = GroupLinearUnit(width, width_act, num_head, keys[0])
        scale = 1.0 / np.sqrt(DIM_HEAD * SCALE_GRAPH)
        self.kernel = jax.random.normal(keys[1], (num_head, DIM_HEAD * SCALE_GRAPH, DIM_HEAD)) * scale
        self.glu    = GatedLinearUnit(width_norm, width_norm, num_head, SCALE_GRAPH, keep_groups=True, key=keys[2])

        print("##params[virt]:", _count_params(self))

    def __call__(self, x, virt, batch, batch_size, node_mask, key=None):
        xx = self.linear(x)
        msg = _segment_logsumexp(xx, batch, batch_size, node_mask)
        if virt is None:
            virt = msg
        else:
            msg = virt = jnp.logaddexp(msg, virt)
        msg = msg.reshape(*msg.shape[:-1], self.num_head, -1)
        msg = jnp.einsum("...hd,hdf->...hf", msg, self.kernel)
        msg = msg.reshape(*msg.shape[:-2], -1)
        msg = self.glu(msg, key=key)
...
class MixerBlock(eqx.Module):
    ...
    def __init__(self, width, num_head, edge_dims_per_hop, key=None):
        ...
        self.virt = VirtBlock(width, num_head, key=keys[-2])
        # scale shape: (num_messages, num_head)
        self.scale = jnp.zeros((len(edge_dims_per_hop) + 1, num_head), dtype=jnp.float32)

    def __call__(self, x, virt, edges, batch, batch_size, node_mask, key=None):
        ...
        xx, virt = self.virt(x, virt, batch, batch_size, node_mask, key=keys[len(self.conv)])
        msgs.append(xx)
        
        # msgs: list of (N, num_head * DIM_HEAD)
        # scale: (num_messages, num_head)
        scale = jax.nn.softmax(self.scale.clip(None, 0), axis=0)
        ...
        scale_expanded = jnp.repeat(scale_norm, DIM_HEAD, axis=-1)
        
        return sum(s * m for s, m in zip(scale_expanded, msgs)), virt

class HeadBlock(eqx.Module):
    ...
    def __init__(self, width, num_head, key=None):
        ...
        self.linear = GroupLinearUnit(width, width_act, num_head, keys[0])
        scale = 1.0 / np.sqrt(DIM_HEAD * SCALE_GRAPH)
        self.kernel = jax.random.normal(keys[1], (num_head, DIM_HEAD * SCALE_GRAPH, DIM_HEAD*2)) * scale
        self.glu    = GatedLinearUnit(width_norm*2, 1, num_head*2, SCALE_GRAPH, key=keys[2])

        print("##params[head]:", _count_params(self))

    def __call__(self, x, virt, batch, batch_size, node_mask, key=None):
        xx = self.linear(x)
        yy = _segment_logsumexp(xx, batch, batch_size, node_mask)
        yy = jnp.logaddexp(yy, virt)
        yy = yy.reshape(*yy.shape[:-1], self.num_head, -1)
        yy = jnp.einsum("...hd,hdf->...hf", yy, self.kernel)
...
class MetaGIN(eqx.Module):
    """MetaGIN for PCQM4Mv2: 19-feature atoms, 6-feature bonds, 8-step RWPE."""
    depth: int
    width: int
    num_head: int
    ...
    atom_pos: eqx.nn.Linear

    mixs: tuple  # depth MixerBlocks
    meta: tuple  # depth MetaFormerUnits wrapping middle mixers

    head: HeadBlock

    def __init__(self, depth, width, num_head, key=None):
        if key is None:
        ...
        self.atom_pos = eqx.nn.Linear(EMBED_POS, width, use_bias=False, key=keys[curr]); curr += 1

        mixs = []
        meta = []
        for i in range(depth):
            mixs.append(MixerBlock(width, num_head, EDGE_DIMS_PER_HOP, key=keys[curr]))
            curr += 1
            meta.append(MetaFormerUnit(width, num_head, SCALE_NODE, key=keys[curr]))
            curr += 1
        self.mixs = tuple(mixs)
        self.meta = tuple(meta)

        self.head = HeadBlock(width, num_head, key=keys[curr])
    ...
    def __call__(self, batch, training=False, key=None):
        ...
        h = h + jax.vmap(self.atom_pos)(z_rw)

        # Split keys for each block, and further split for sub-components (mixer + wrapper)
        keys = _split_or_none(key if training else None, (self.depth + 2) * 2)

        # Blocks 1 to self.depth: Mixer wrapped by MetaFormer
        virt = None
        for i in range(self.depth):
            msg, virt = self.mixs[i](h, virt, edges, graph_id, batch_size, node_mask, key=keys[2*i])
            h = self.meta[i](h, msg, key=keys[2*i+1])

        return self.head(h, virt, graph_id, batch_size, node_mask)[1:]
    ...
    def _get_edge(self, batch):
        ...
        for suffix in EDGE_SUFFIXES:
            ...
            deg = _segment_sum(jnp.ones((edge_mask.shape[0], 1), dtype=edge_index.dtype), edge_index[1], num_nodes, edge_mask).squeeze(-1).clip(1, None)
            edges.append((edge_index, edge_attr, deg, edge_mask))
        return edges


def get_model(key):
    return MetaGIN(depth=6, width=256, num_head=16, key=key)
