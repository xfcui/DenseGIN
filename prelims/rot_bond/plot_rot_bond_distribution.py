...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            ...
            "(2) number of non-rotatable bonds; "
            "(3) number of rotatable bonds vs. heavy atoms."
        )
    )
    parser.add_argument(
        "--raw-csv-path",
    ...
    parser.add_argument(
        "--out",
        type=str,
        default="rot_bond_joint_10k.svg",
        help=(
            "Output path for the rotatable vs non-rotatable bond plot (SVG). "
            "The rotatable-bond-vs-heavy-atom plot is saved alongside it with a "
            "'_rotatable_vs_heavy' suffix."
        ),
    )
...
def count_rotatable_and_non_rotatable_bonds(smiles: str) -> tuple[int, int] | None:
    """Return (rotatable_bonds, non_rotatable_bonds), or None if SMILES is invalid."""
...
def count_rotatable_and_heavy_atoms(smiles: str) -> tuple[int, int] | None:
    """Return (rotatable_bonds, heavy_atoms), or None if SMILES is invalid."""
    mol = Chem.MolFromSmiles(smiles)
    if mol is None:
        return None
    mol = Chem.AddHs(mol)
...
def plot_rotatable_vs_non_rotatable(
    rot_counts: np.ndarray,
    non_rot_counts: np.ndarray,
    out_path: str,
) -> str:
    return _render_joint_svg(
        x_counts=non_rot_counts,
        y_counts=rot_counts,
        out_path=out_path,
        title="Rotatable Bonds vs Non-Rotatable Bonds",
        x_label="Number of non-rotatable bonds per molecule",
        y_label="Number of rotatable bonds per molecule",
        x_marginal_label="Non-rotatable-bond marginal distribution",
        y_marginal_label="Rotatable-bond marginal distribution",
        x_bar_color="#4c78a8",
        y_bar_color="#e4572e",
...
def plot_rotatable_vs_heavy(
    ...
    return _render_joint_svg(
        ...
        y_counts=rot_counts,
        out_path=out_path,
        title="Rotatable Bonds vs Heavy Atoms",
...
def main():
    ...
    for smiles in smiles_samples:
        bond_counts = count_rotatable_and_non_rotatable_bonds(smiles)
        if bond_counts is None:
            invalid_smiles += 1
            continue
        heavy_data = count_rotatable_and_heavy_atoms(smiles)
        if heavy_data is None:
            invalid_smiles += 1
            continue
        rot_counts.append(bond_counts[0])
        non_rot_counts.append(bond_counts[1])
        heavy_counts.append(heavy_data[1])

    if not rot_counts:
        raise ValueError("No valid molecules found in sampled SMILES.")

    rot_counts = np.asarray(rot_counts, dtype=np.int32)
    non_rot_counts = np.asarray(non_rot_counts, dtype=np.int32)
    heavy_counts = np.asarray(heavy_counts, dtype=np.int32)

    out_path = _resolve_output_path(args.out)
    path = plot_rotatable_vs_non_rotatable(rot_counts, non_rot_counts, out_path)
    path2 = plot_rotatable_vs_heavy(
        rot_counts,
        heavy_counts,
        _sibling_path(out_path, "_rotatable_vs_heavy"),
    )

    print(f"Saved rotatable vs non-rotatable bond plot to: {path}")
    print(f"Saved rotatable-bond vs heavy-atom plot to:    {path2}")
    print(f"Requested samples:               {len(smiles_samples)}")
    print(f"Valid molecules used:            {len(rot_counts)}")
    print(f"Invalid SMILES skipped:          {invalid_smiles}")
    print(f"Mean rotatable bonds:            {rot_counts.mean():.4f}")
    print(f"Mean non-rotatable bonds:        {non_rot_counts.mean():.4f}")
    print(f"Mean heavy atoms:                {heavy_counts.mean():.4f}")
    print(f"Median rotatable bonds:          {float(np.median(rot_counts)):.1f}")
    print(f"Median non-rotatable bonds:      {float(np.median(non_rot_counts)):.1f}")
    print(f"Median heavy atoms:              {float(np.median(heavy_counts)):.1f}")
    print(f"Max rotatable bonds:             {rot_counts.max()}")
    print(f"Max non-rotatable bonds:         {non_rot_counts.max()}")
